using UnityEngine;

public class Constants
{

    public static readonly float DEFAULT_WALK_SPEED = 0.5f;
    public static readonly float VIRTICAL_WALK_SPEED = 0.3f;
    public static readonly float DO_MOVE_X_SEC = 0.5f;
    public static readonly float DO_MOVE_Z_SEC = 1.0f;
    public static readonly float VERTICAL_DEG = 30;
    public static readonly float HORIZONTAL_DEG = 80;

    public static readonly float CHARACTER_FOLLOW_OFFSET = 3;
    public static readonly Vector3 WALK_CAM_POS = new Vector3(0, 7.23f, -17.61f);

    public static readonly float BUTTLE_START_SEC = 0.3f;
    public static readonly float BUTTLE_DEG = 45;

    public static readonly Vector3 BUTTLE_CAM_OFFSET = new Vector3(0, 4, 6);
}
